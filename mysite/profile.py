...
def setUserVoice(user_id, photo):
    ...
    FH = open("./Dat/voice.dat", "r") 
    line = FH.readline()  
    voice_id = int(line) + 1
    FH.close()
    FH = open("./Dat/voice.dat", "w")
    FH.write(str(voice_id))
    FH.close()

    # Create a file object in "write" mode  
    FH = open("./Voice/" + str(voice_id), "wb")  
...
def setUserPhoto(user_id, photo):
    ...
    FH = open("./Dat/photo.dat", "r") 
    line = FH.readline()  
    photo_id = int(line) + 1
    FH.close()
    FH = open("./Dat/photo.dat", "w")
    FH.write(str(photo_id))
    FH.close()

    # Create a file object in "write" mode  
    FH = open("./Photo/" + str(photo_id), "wb")  
    # Write all the lines at once to the file handle  
    FH.write(photo)    
    FH.close()      
...
def getUserPhoto(user_id):
    ...
    conn.close()
    photo_id = a[0][3]
    return "/nfs/home/haoen0514/djangogirls/mysite/Photo/" + photo_id
...
def getUserVoice(user_id):
    ...
    conn.close()
    voice_id = a[0][4]
    return "/nfs/home/haoen0514/djangogirls/mysite/Voice/" + voice_id
